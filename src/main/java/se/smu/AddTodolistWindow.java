...
public class AddTodolistWindow extends JFrame{
	...
	public AddTodolistWindow() {
		...
		JLabel title = new JLabel("To do list 등록 (To-do list 翻译)");
		c.add(title);
		...
		JLabel todoName = new JLabel("항목이름(项目名称)");
		...
		JLabel todoFinishDate = new JLabel("실제마감일(实际截止日期)");
		...
		JLabel todoCompleted = new JLabel("완료여부(状态完成)");
		...
		JLabel todoImportant = new JLabel("중요여부(也罢)");
		...
		JButton todoConfirm = new JButton("등록(注册)");
		c.add(todoConfirm);
